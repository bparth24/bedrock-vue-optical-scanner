 ...
 * 
 * Renderless Vue component that bridges Vue reactivity with bedrock-web-optical-scanner.
...
export default {
  ...
  setup(props, { emit, expose }) {
    ...
    function createScanner() {
      ...
      if (plugins.length === 0) {
        console.warn('ScanningOrchestrator: No valid plugins found for formats:', props.formats);
        return null;
      }
    ...
    async function performScan(sourceToScan = null, options = {}) {
      ...
      try {
        scanning.value = true;
        scanningFormat.value = props.formats[0]; // Show first format as scanning
    ...
    function startContinuousScanning() {
      if (continuousTimer || !props.continuous) return;
    ...
    watch(() => props.source, (newSource, oldSource) => {
      ...
      if (props.continuous && newSource && newSource !== oldSource && !scanning.value) {
        nextTick(() => {
          performScan();
        });
      }
...
<template>
  <!-- This component is renderless - all logic, no UI -->
</template>